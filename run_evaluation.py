...
class EvaluateAll:
    ...
    def run_evaluation(self):
        ...
        import detectors.cascade_detector.detectorleft as left_detector
        import detectors.cascade_detector.detectorright as right_detector
        
        # import detectors.your_super_detector.detector as super_detector
        cascade_detector = cascade_detector.Detector()
        left_detector = left_detector.DetectorOfLeftEar()
        right_detector = right_detector.DetectorOfRightEar()
        ...
        for im_name in im_list:
            ...
            img = preprocess.histogram_equlization_rgb(preprocess.brightnessCorrection(img)) # This one makes VJ worse
            ...
            prediction_list = right_detector.detect(img)
            ...
            f1score = eval.fscore_compute(p, gt)
            
            iou_arr.append(iou)
            f1score_arr.append(f1score)

        miou = np.average(iou_arr)
        mf1score = np.average(f1score_arr)
        print("\n")
        print("Average IOU:", f"{miou:.2%}")
        
        print("Average F1-score:", f"{mf1score:.2%}")
